...
class NatNetClient:
    def __init__( self,localIP,serverIP , IDS):
        # Change this value to the IP address of the NatNet server.
        self.serverIPAddress = localIP

        # Change this value to the IP address of your local network interface
        self.localIPAddress = serverIP  #"10.8.244.172" 
    ...
    def __unpackRigidBody( self, data ):
        ...
        for str_id in range(len(self.IDS)):

            if self.rigidBodyListener is not None  and id == self.IDS[str_id]:
           #print('a')
                  self.rigidBodyListener[str_id] = [float(id), np.array(pos), np.array(rot)]
                  #self.rigidBodyListener[str_id]= np.array([float(id), np.array(pos), np.array(rot)])
    ...
    def __unpackMocapData( self, data ):
        ...
        for i in range( 0, unlabeledMarkersCount ):
            ...
            offset += 12
            trace( "\tMarker", i, ":", pos[0],",", pos[1],",", pos[2] )
            unlabeledMarkerPositions.append( pos )
        ...
        for i in range( 0, rigidBodyCount ):
            _= self.__unpackRigidBody( data[offset:] )
            offset += _
        ...
        if self.newFrameListener is not None:
            self.f_n = self.newFrameListener( frameNumber, markerSetCount, unlabeledMarkersCount, rigidBodyCount, skeletonCount,
                                  labeledMarkerCount, timecode, timecodeSub, timestamp, isRecording, trackedModelsChanged,f_n )

        if self.markerListener is not None:
            self.markerListener( frameNumber, unlabeledMarkerPositions )
